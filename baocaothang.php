...
if ($_GET['s'] && $_GET['f']) {
		...
		if ($_GET['from'] && $_GET['to']){
			...
			// ISO2: Tạo điều kiện loại trừ thiết bị tạm dừng (ngoại trừ tạm dừng trong tháng báo cáo)
			$exclude_tamdung = "AND (hoso NOT IN (SELECT hoso FROM hososcbd_tamdung WHERE trangthai='dang_tam_dung' AND id IN (SELECT MAX(id) FROM hososcbd_tamdung GROUP BY hoso)) 
			                    OR hoso IN (SELECT hoso FROM hososcbd_tamdung WHERE trangthai='dang_tam_dung' AND id IN (SELECT MAX(id) FROM hososcbd_tamdung GROUP BY hoso)
			                                AND YEAR(ngay_tamdung)='$yto' AND MONTH(ngay_tamdung)='$mto'))";
			...
			$exclude_tamdung = "AND (hoso NOT IN (SELECT hoso FROM hososcbd_tamdung WHERE trangthai='dang_tam_dung' AND id IN (SELECT MAX(id) FROM hososcbd_tamdung GROUP BY hoso)) 
			                    OR hoso IN (SELECT hoso FROM hososcbd_tamdung WHERE trangthai='dang_tam_dung' AND id IN (SELECT MAX(id) FROM hososcbd_tamdung GROUP BY hoso)
			                                AND YEAR(ngay_tamdung)='$y' AND MONTH(ngay_tamdung)='$m'))";
