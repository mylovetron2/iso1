...
$sql_maql = mysql_query("SELECT DISTINCT maql FROM hososcbd_iso WHERE $where ORDER BY maql");
$tong_maql = 0;
$tong_somay = 0;
$tong_ts_gio = 0;
$stt = 1;
...
while($row_maql = mysql_fetch_array($sql_maql)) {
  ...
  while($row_hoso = mysql_fetch_array($sql_hoso)) {
    ...
    $sql_nv = mysql_query("SELECT n.*, h.maql, h.mavt, h.somay, h.ttktafter, h.honghoc, h.cv, h.madv FROM ngthuchien_iso n LEFT JOIN hososcbd_iso h ON n.mahoso = h.hoso WHERE n.mahoso='$hoso'");
    $tenvt = '';
    ...
    if ($tong_gio > 0) {
      ...
      echo '<td style="text-align:left;padding-left:8px">'.htmlspecialchars(implode(", ", $ds_nv_short)).'</td>';
      echo '<td style="text-align:center">'.$row_hoso['ttktafter'].'</td>';
