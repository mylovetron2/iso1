...
// Biến xử lý tạm dừng/tiếp tục
$action_tamdung = isset($_POST['action_tamdung']) ? $_POST['action_tamdung'] : '';
$lydo_tamdung = isset($_POST['lydo_tamdung']) ? $_POST['lydo_tamdung'] : '';
...
$result2 = mysql_query("SELECT ghichufinal FROM hososcbd_iso WHERE hoso='$edithoso'") ;
...
// LOG FIRST - Test if we reach this block
$log_file = __DIR__ . '/debug_bddk.log';
file_put_contents($log_file, date('Y-m-d H:i:s') . " - FORM SUBMITTED (xuatphieu=phieukiemtra)\n", FILE_APPEND);
...
echo "cv = '$cv'<br>";
if(isset($_POST['cv'])) {
	echo "_POST['cv'] exists = '" . $_POST['cv'] . "'<br>";
} else {
	echo "_POST['cv'] DOES NOT EXIST<br>";
}
echo "</div>";
*/
...
// Convert ngaykt to Y-m-d format after parsing
if($ngayt != "" && $thangt != "" && $namt != "") {
	$ngaykt = "$namt-$thangt-$ngayt";
	file_put_contents($log_file, "Converted ngaykt format to: $ngaykt\n", FILE_APPEND);
}
...
// LOG AFTER PARSING DATES  
file_put_contents($log_file, "After date parse - ngayth: $ngayth, ngays=$ngays, thangs=$thangs, nams=$nams\n", FILE_APPEND);
file_put_contents($log_file, "After date parse - ngaykt: '$ngaykt', ngayt=$ngayt, thangt=$thangt, namt=$namt\n", FILE_APPEND);
file_put_contents($log_file, "Device info - mavt=$mavt, somay=$somay, model=$model\n", FILE_APPEND);
...
// Xử lý cập nhật kế hoạch BDDK khi checkbox BDDK được chọn
// Debug: Ghi log vào file
$log_file = __DIR__ . '/debug_bddk.log';
...
if($bddk == 'BDDK') {
	...
	$num_rows = mysql_num_rows($thietbi_id_query);
	file_put_contents($log_file, "Query rows: $num_rows\n", FILE_APPEND);
	
	if($thietbi_row = mysql_fetch_array($thietbi_id_query)) {
		$thietbi_id_bddk = $thietbi_row['stt'];
		
		file_put_contents($log_file, "Found thietbi_id: $thietbi_id_bddk\n", FILE_APPEND);
		
		// Xác định quý dựa trên tháng
		$thang_bd = (int)$thangs;
		$nam_bd = (int)$nams;
		$quy = ceil($thang_bd / 3); // Quý 1: 1-3, Quý 2: 4-6, Quý 3: 7-9, Quý 4: 10-12
		
		file_put_contents($log_file, "Month: $thang_bd, Year: $nam_bd, Quarter: $quy\n", FILE_APPEND);
		...
		// Xác định trạng thái hoàn thành dựa vào ngày BĐ (ngày thực hiện)
		// Nếu có ngày thực hiện hợp lệ → đã hoàn thành
		$ngayth_formatted = "$nams-$thangs-$ngays"; // Y-m-d format
		file_put_contents($log_file, "ngayth formatted: '$ngayth_formatted'\n", FILE_APPEND);
		
		$is_valid_start_date = ($ngayth_formatted != "0000-00-00" && $ngayth_formatted != "00-00-0000" && $ngayth_formatted != "--" && $ngayth_formatted != "" && $ngays != "" && $thangs != "" && $nams != "");
		$hoantat = $is_valid_start_date ? 1 : 0;
		
		file_put_contents($log_file, "Using ngayth (start date) for hoantat: $hoantat\n", FILE_APPEND);
		
		$check_count = mysql_num_rows($check_bddk);
		file_put_contents($log_file, "Existing plans: $check_count, hoantat: $hoantat\n", FILE_APPEND);
		
		if(mysql_num_rows($check_bddk) > 0) {
			// Đã có kế hoạch, UPDATE
			$bddk_row = mysql_fetch_array($check_bddk);
			$bddk_id = $bddk_row['id'];
			
			// LOG: Check current values
			$check_current = mysql_query("SELECT * FROM ke_hoach_bao_duong_dinh_ky_iso WHERE id='$bddk_id'");
			if($current_row = mysql_fetch_array($check_current)) {
				file_put_contents($log_file, "Current record id=$bddk_id: qui_$quy='" . $current_row["qui_$quy"] . "', qui_{$quy}_hoantat=" . $current_row["qui_{$quy}_hoantat"] . "\n", FILE_APPEND);
			}
			
			$update_bddk = "UPDATE ke_hoach_bao_duong_dinh_ky_iso SET 
				qui_$quy = 'TO',
				qui_{$quy}_hoantat = $hoantat,
				nhomsc = '$nhomsc'
				WHERE id = '$bddk_id'";
			
			file_put_contents($log_file, "UPDATE QUERY: $update_bddk\n", FILE_APPEND);
			file_put_contents($log_file, "New values - qui_$quy='TO', qui_{$quy}_hoantat=$hoantat\n", FILE_APPEND);
			
			$result = mysql_query($update_bddk);
			if(!$result) {
				file_put_contents($log_file, "UPDATE ERROR: " . mysql_error() . "\n", FILE_APPEND);
			} else {
				$affected = mysql_affected_rows();
				file_put_contents($log_file, "UPDATE affected rows: $affected\n", FILE_APPEND);
				
				// If no rows affected, check if values are already the same
				if($affected == 0) {
					file_put_contents($log_file, "WARNING: No rows updated. Values may already be the same.\n", FILE_APPEND);
				}
			}
			
			// Thông báo thành công
			// echo "<script>alert('Đã cập nhật kế hoạch BDDK - Quý $quy năm $nam_bd');</script>";
		} else {
			// Chưa có kế hoạch, INSERT mới
			// Lấy tên thiết bị và số serial
			$tb_info = mysql_query("SELECT tenvt FROM thietbi_iso WHERE stt='$thietbi_id_bddk'");
			$ten_tb = "";
			if($tb_row = mysql_fetch_array($tb_info)) {
				$ten_tb = $tb_row['tenvt'];
			}
			
			$insert_bddk = "INSERT INTO ke_hoach_bao_duong_dinh_ky_iso (
				thietbi_id,
				nam,
				ten_thietbi,
				so_serial,
			...
			file_put_contents($log_file, "INSERT QUERY: $insert_bddk\n", FILE_APPEND);
			
			mysql_query($insert_bddk) or die("Lỗi INSERT BDDK: " . mysql_error());
			
			$insert_id = mysql_insert_id();
			file_put_contents($log_file, "INSERT completed, insert id: $insert_id\n", FILE_APPEND);
			
			// Thông báo thành công
			// echo "<script>alert('Đã tạo mới kế hoạch BDDK - Quý $quy năm $nam_bd');</script>";
		}
	} else {
...
$tenthietbisql11 = mysql_query("SELECT DISTINCT stt,mamay,homay,dienap FROM thietbi_iso where mavt='$mavt' and somay='$somay' and model='$model'") ;
		while($row = mysql_fetch_array($tenthietbisql11))
		{
			$thietbi_id =$row['stt'];
			$mamay =$row['mamay'];	
...
for(var i=0; i < cvCheckboxes.length; i++) {
	cvCheckboxes[i].addEventListener('click', function() {
		if(this.checked) {
			for(var j=0; j < cvCheckboxes.length; j++) {
				if(cvCheckboxes[j] !== this) {
					cvCheckboxes[j].checked = false;
				}
			}
		}
	});
...
if(isset($thietbi_id) && $thietbi_id > 0) {
	...
	if($bddk_row = mysql_fetch_array($bddk_query)) {
		...
		$qui_3 = $bddk_row['qui_3'];
		$qui_4 = $bddk_row['qui_4'];
		$qui_1_ht = $bddk_row['qui_1_hoantat'];
		$qui_2_ht = $bddk_row['qui_2_hoantat'];
		$qui_3_ht = $bddk_row['qui_3_hoantat'];
		$qui_4_ht = $bddk_row['qui_4_hoantat'];
		$ghi_chu_bddk = $bddk_row['ghi_chu'];
		
		echo "<table class=\"table1\" style=\"margin-left:50px;width:800px;margin-top:10px;\">
		<tr>
		<th colspan=\"5\" style=\"text-align:center;font-size: 16;background-color:#20B2AA;\">THÔNG TIN BẢO DƯỠNG ĐỊNH KỲ - NĂM $nam_bddk</th>
		</tr>
		<tr>
		<th style=\"text-align:center;font-size: 15;width:100px;\">Quý 1</th>
		<th style=\"text-align:center;font-size: 15;width:100px;\">Quý 2</th>
		<th style=\"text-align:center;font-size: 15;width:100px;\">Quý 3</th>
		<th style=\"text-align:center;font-size: 15;width:100px;\">Quý 4</th>
		<th style=\"text-align:center;font-size: 15;width:400px;\">Ghi chú</th>
		</tr>
		<tr>
		<td style=\"text-align:center;".($qui_1 == 'TO' ? "background-color:#FFA500;" : "")."\">";
		if($qui_1 == 'TO') {
			if($qui_1_ht == 1) echo "<span style=\"color:green;font-size:18px;\">✓</span>";
			else echo "&nbsp;";
		} else {
			echo "-";
		}
		echo "</td>
		<td style=\"text-align:center;".($qui_2 == 'TO' ? "background-color:#FFA500;" : "")."\">";
		if($qui_2 == 'TO') {
			if($qui_2_ht == 1) echo "<span style=\"color:green;font-size:18px;\">✓</span>";
			else echo "&nbsp;";
		} else {
			echo "-";
		}
		echo "</td>
		<td style=\"text-align:center;".($qui_3 == 'TO' ? "background-color:#FFA500;" : "")."\">";
		if($qui_3 == 'TO') {
			if($qui_3_ht == 1) echo "<span style=\"color:green;font-size:18px;\">✓</span>";
			else echo "&nbsp;";
		} else {
			echo "-";
		}
		echo "</td>
		<td style=\"text-align:center;".($qui_4 == 'TO' ? "background-color:#FFA500;" : "")."\">";
		if($qui_4 == 'TO') {
			if($qui_4_ht == 1) echo "<span style=\"color:green;font-size:18px;\">✓</span>";
			else echo "&nbsp;";
		} else {
			echo "-";
		}
		echo "</td>
		<td style=\"padding-left:10px;\">$ghi_chu_bddk</td>
		</tr>
		</table><br/>";
	} else {
		...
		<tr>
		<td style=\"text-align:center;color:red;font-size:16px;font-weight:bold;padding:20px;\">⚠ Không xác định được kế hoạch bảo dưỡng định kỳ</td>
		</tr>
		</table><br/>";
	}
} else {
